...
public class Parser {
	...
	private Parser(Builder builder) {
		this.blockParserFactories = DocumentParser
				.calculateBlockParserFactories(builder.blockParserFactories,
	...
	public Node parse(String input) {
		...
		DocumentParser documentParser = new DocumentParser(blockParserFactories,
				inlineParser);
	...
	private InlineParser getInlineParser() {
		if (this.inlineParserFactory == null) {
			...
			CustomInlineParserContext inlineParserContext = new CustomInlineParserContext(
					delimiterProcessors);
	...
	public static class Builder {
		...
		private Set<Class<? extends Block>> enabledBlockTypes = DocumentParser
		 ...
		 * </pre>
		 *
		 * @param enabledBlockTypes
		 *            A list of block nodes the parser will parse. If this list is empty, the parser will not recognize any CommonMark core features.
		 * @return {@code this}
		 */
		public Builder enabledBlockTypes(
		 ...
		 * <p>
		 * Note that custom factories are applied <em>before</em> the built-in factories. This is so that extensions can change how some syntax is parsed that would otherwise be handled by built-in factories. "With great power comes great responsibility."
		 *
		 * @param blockParserFactory
		 *            a block parser factory implementation
		 * @return {@code this}
		 */
		public Builder customBlockParserFactory(
		 ...
		 * <p>
		 * Note that multiple delimiter processors with the same characters can be added, as long as they have a different minimum length. In that case, the processor with the shortest matching length is used. Adding more than one delimiter processor with the same character and minimum length is
		 * invalid.
		 *
		 * @param delimiterProcessor
		 *            a delimiter processor implementation
		 * @return {@code this}
		 */
		public Builder customDelimiterProcessor(
		 ...
		 *
		 * @param inlineParserFactory
		 *            an inline parser factory implementation
		 * @return {@code this}
		 */
		public Builder inlineParserFactory(
